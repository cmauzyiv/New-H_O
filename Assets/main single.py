...
def main():
    ...
    while run:
        ...
        if phase == 1:
            ...
            collided = pygame.sprite.groupcollide(sprite_group, enemy_spawner.asteroid_group, False,
            ...
            if trial_type == 0:
                ...
                collided = pygame.sprite.groupcollide(sprite_group, p2enemy_spawner.sdasteroid_group, False,
                                                      True)  ## seperate badguy1 as alt1 group
                for player, asteroid in collided.items():
                    ##shield_spawner.spawn_shipshield((player.rect.x, player.rect.y))
                    ##shield_spawner.spawn_shipshield((player.rect.x, player.rect.y))
            ...
            if trial_type == 1:
                ...
                for player, new_asteroid in collided.items():
                    ...
                    if new_asteroid[0].is_destroyed:
                        asteroids += 1
                        stream.append(5)
                        explosion_spawner.spawn_Sdeltaasteroidexplosions(
                            (new_asteroid[0].rect.x, new_asteroid[0].rect.y))
                        player.hud.asteroid_score.update_score(new_asteroid[0].point_value)
                ...
                for player, new_asteroid in collided.items():
                    ...
                    if new_asteroid[0].is_destroyed:
                        asteroids += 1
                        stream.append(5)
                        explosion_spawner.spawn_Sdasteroidexplosions((new_asteroid[0].rect.x, new_asteroid[0].rect.y))
                        player.hud.asteroid_score.update_score(new_asteroid[0].point_value)
                collided = pygame.sprite.groupcollide(player.lasers, p2enemy_spawner.enemy_group2, True, False)
                for laser, enemy in collided.items():
                    enemy[0].get_hit()
