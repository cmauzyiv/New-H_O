...
## mask is the layer that it targets
...
@onready var invincible_component: = $InvincibleComponent as InvincibleComponent
@export var game_stats: GameStats
@onready var shield_spawner_component: = $ShieldSpawnerComponent as SpawnerComponent
@onready var shield_animated: AnimatedSprite2D = $Anchor/ShieldAnimated
@onready var move_component:  = $States/MoveDownState/MoveComponent as MoveComponent
...
func _ready() -> void:
	game_stats.phase_changed.connect(func(new_phase: int):
		if new_phase == invincible_component.invincible_phase:
	...
	##hurtbox_component.hurt.connect(func(hitbox: HitboxComponent):
		##shield_animated.play("default")
		##) 
	hurtbox_component.hurt.connect(shield_animated.play_shield.unbind(1))
	hurtbox_component.hurt.connect(score_component.adjust_responses.unbind(1))
...
func enable_move_down_state() -> void:
	...
	move_component.velocity.y = randi_range(50, 125)
